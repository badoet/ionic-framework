...
export class Accordion implements ComponentInterface {
  private accordionGroupEl?: HTMLSygAccordionGroupElement | null;
   ...
   * value.
   */
  @Prop() value = `syg-accordion-${accordionIds++}`;
  ...
  /**
   * The slot inside of `syg-item` to
   * place the toggle icon. Defaults to `'end'`.
   */
  @Prop() toggleIconSlot: 'start' | 'end' = 'end';

  connectedCallback() {
    const accordionGroupEl = this.accordionGroupEl = this.el && this.el.closest('syg-accordion-group');
    if (accordionGroupEl) {
  ...
  componentDidLoad() {
    ...
    raf(() => {
      /**
       * Set aria label on button inside of syg-item
  ...
  private setItemDefaults = () => {
    const ionItem = this.getSlottedHeaderSygItem();
    if (!ionItem) { return; }

    /**
     * For a11y purposes, we make
     * the syg-item a button so users
  ...
  private getSlottedHeaderSygItem = () => {
    const { headerEl } = this;
    if (!headerEl) { return; }

    /**
     * Get the first syg-item
    ...
    const ionItem = slot.assignedElements && (slot.assignedElements().find(el => el.tagName === 'SYG-ITEM') as HTMLSygItemElement | undefined);
  ...
  private setAria = (expanded = false) => {
     ...
     * syg-item because that is what will be focused
     */
  ...
  private slotToggleIcon = () => {
     ...
     */
    const existingToggleIcon = ionItem.querySelector('.syg-accordion-toggle-icon');
    if (existingToggleIcon) { return; }

    const iconEl = document.createElement('ion-icon');
    iconEl.slot = toggleIconSlot;
    iconEl.lazy = false;
    iconEl.classList.add('syg-accordion-toggle-icon');
  ...
  private getNextSibling = () => {
    ...
    if (nextSibling?.tagName !== 'SYG-ACCORDION') { return; }

    return nextSibling as HTMLSygAccordionElement;
  }
  ...
  private getPreviousSibling = () => {
    ...
    if (previousSibling?.tagName !== 'SYG-ACCORDION') { return; }

    return previousSibling as HTMLSygAccordionElement;
  }

...
export const startInputShims = (config: Config) => {
  ...
  const inputs = Array.from(doc.querySelectorAll('syg-input, syg-textarea')) as HTMLElement[];
  ...
  const registerInput = async (componentEl: HTMLElement) => {
    ...
    const scrollEl = componentEl.closest('syg-content');
    const footerEl = (!scrollEl) ? componentEl.closest('syg-footer') as HTMLSygFooterElement | null : null;
  ...
  // Input might be already loaded in the DOM before syg-device-hacks did.
